...
#define ITERATIONS (1<<5) // iterations are multiplied with 64

void benchmark_dataset_generation(uint32_t height, uint64_t* dataset){
	char      buffer[65] = {0};
...
uint64_t benchmark_mine(uint32_t block_height, uint8_t printing){
	...
	if(printing){
		benchmark_dataset_generation(block_height, dataset_64);
	} else {
		dataset_from_height(block_height, dataset_64);
	}
...
uint64_t benchmark_validation(uint32_t block_height, uint8_t printing){
	uint64_t  result_64[4] = {0};
	uint8_t*  result       = (uint8_t*)result_64;
	uint64_t  temp[4]      = {0};
	uint8_t   header[88]   = {0};
	...
	for(uint16_t i=0;i<64;i++) buffer[i]=' ';
	current_time = (uint32_t)time(NULL);
	cache_from_height(block_height, cache);
