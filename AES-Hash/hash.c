...
#define ITER  1024      // Number of iterations to encrypt
...
int main(){
    ...
    uint64_t  salt_64[4] = {0xb40bbe37c30c8ea1,0x2d02ef8d53b39330,
                            0x7bb12bae9b64c2b0,0xd3d6f4fb6c0695ed};
    uint8_t*  salt       = (uint8_t*)salt_64;
    uint32_t  stime      = 0;
    uint32_t  etime      = 0;
    uint32_t  size       = SIZE;
    double    throughput = 0;
    ...
    for(uint32_t i=0;i<ITER;i++) hash(msg, size, salt);
    etime = (uint32_t)time(NULL);
    etime = etime-stime;

    printf("Calculation of %u iterations of encrypting %u bytes took: %us\n",ITER,size,etime);
...
# endif
    printf("Result: %016jx.%016jx:%016jx.%016jx\n",salt_64[0],salt_64[1],salt_64[2],salt_64[3]);
}
#endif
