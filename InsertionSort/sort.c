...
int partition(uint32_t A[], uint32_t p, uint32_t q) {
  swap(&A[p + (rand() % (q - p + 1))], &A[q]);   // PIVOT = A[q]
 
  uint32_t i = p - 1;
  for(uint32_t j = p; j <= q; j++) {
    if(A[j] <= A[q]) {
      swap(&A[++i], &A[j]);
    }
  }
...
void quicksort(uint32_t A[], uint32_t p, uint32_t q) {
  if(p < q) {
    uint32_t pivotIndx = partition(A, p, q);
...
int main(){
	uint32_t arr[LEN] = {10};
	for(uint32_t i=0;i<LEN;i++) crc32p(&arr[i],&arr[i+1]);
	quicksort(arr, 250, 250);
	for(uint32_t i=0;i<LEN;i++) printf("%08x,",arr[i]);
	printf("\n");
