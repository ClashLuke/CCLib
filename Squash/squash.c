...
void squash(uint8_t* data, uint8_t* out){
	uint8_t   shift[4]   = {0};
	uint32_t* data_32    = (uint32_t*)data;
	uint64_t* data0_64   = (uint64_t*)data;
	uint64_t* data1_64   = (uint64_t*)&data[32];
	uint32_t* out_32     = (uint32_t*)out;
	uint64_t* out_64     = (uint64_t*)out;
	out_64[2] = data0_64[2]; out_64[3] = data0_64[3];
	crc32p(*data_32, *out_32);
	crc32p(data_32[1], out_32[1]);
	crc32p(data_32[2], out_32[2]);
	crc32p(data_32[3], out_32[3]);
	out_64[2]+= data0_64[2]; out_64[3]+= data0_64[3];
	*shift = out[31]&0x3f;
	shift[2] = (*out)&0x3f;
	shift[1] = 64-(*shift);
	shift[3] = 64-shift[2];
	*data1_64   = ((*out_64)>>(*shift)) | ((*out_64)<<(shift[1]));
	data1_64[1] = (out_64[1]>>shift[0]) | (out_64[1]<<(shift[1]));
	data1_64[2] = (out_64[2]<<shift[2]) | (out_64[2]>>(shift[3]));
	data1_64[3] = (out_64[3]<<shift[2]) | (out_64[3]>>(shift[3]));
	aes(out     ,  data);
	aes(&out[16], &data[16]);
	return;
}
